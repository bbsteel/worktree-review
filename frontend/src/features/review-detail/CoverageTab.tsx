    ...
    <div className="flex flex-col gap-4">
      <section className="rounded-lg border border-border bg-surface p-4">
        <div className="flex flex-wrap items-center gap-2">
          <h2 className="text-sm font-semibold text-text-primary">Required Coverage</h2>
          {coverage.requiredCoverage === 'complete' ? (
            <Badge tone="passed" label="Complete" />
          ) : (
            <Badge tone="error" label="Incomplete" />
          )}
        </div>
        <p className="mt-2 text-sm text-text-secondary">
          {coverage.reviewedCount} reviewed · {coverage.missingCount} missing ·{' '}
        ...
        {coverage.requiredCoverage === 'incomplete' ? (
          <p role="alert" className="mt-2 text-sm text-status-error">
            ...
            {run.failure !== null
              ? ` The failure was recorded at stage ${PIPELINE_STAGE_LABEL[run.failure.stage]}.`
              : ''}
          </p>
        ) : null}
      </section>
      ...
      {CATEGORY_ORDER.map((category) => {
        ...
        return (
          <section
            key={category}
            aria-label={`${presentation.label} files`}
            className="rounded-lg border border-border bg-surface p-4"
          >
            <div className="flex flex-wrap items-center gap-2">
              <Badge tone={CATEGORY_BADGE_TONE[category]} label={presentation.label} />
              <span className="text-meta text-text-secondary">
                {files.length} file{files.length === 1 ? '' : 's'} — {presentation.description}
              </span>
            </div>
                ...
                >
                  ...
                  {category !== 'reviewed' ? (
                                    <p className="mt-1 text-meta text-text-secondary">
                                      {file.reason ?? 'Reason not reported by this result schema.'}
                                    </p>
                  ) : null}
                  {file.rule !== null ? (
