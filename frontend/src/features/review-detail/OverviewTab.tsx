  ...
  return (
    <div className="flex flex-col gap-4">
      <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 xl:grid-cols-4">
        <SummaryTile title={t('Gate Decision')}>
          <Badge tone={gate.tone} label={t(gate.label)} />
        </SummaryTile>

        <SummaryTile title={t('Blocking Findings')}>
          {gateEvaluated ? (
            <p className="text-lg font-semibold text-text-primary tabular-nums">
              {blockingCount}
              <span className="ml-1 text-sm font-normal text-text-secondary">
                {t(blockingCount === 1 ? 'finding' : 'findings')}
              </span>
            </p>
          ) : (
            <p className="text-sm text-text-secondary">
              {t('Not evaluated — the review did not reach gate evaluation.')}
            </p>
          )}
        </SummaryTile>

        <SummaryTile title={t('Required Coverage')}>
          <p className="text-sm font-medium text-text-primary">
            {t(run.coverage.requiredCoverage === 'complete' ? 'Complete' : 'Incomplete')}
          </p>
          <p className="text-meta text-text-secondary">
            {run.coverage.reviewedCount} {t('reviewed')} · {run.coverage.missingCount} {t('missing')} ·{' '}
            {run.coverage.excludedCount} {t('excluded')}
          </p>
        </SummaryTile>

        <SummaryTile title={t('Cost (estimated / actual)')}>
          <p className="text-sm font-medium text-text-primary tabular-nums">
            {t(formatCostUsd(run.usage.estimatedCostUsd, run.usage.costUnknown))} /{' '}
            {t(formatCostUsd(run.usage.actualCostUsd, run.usage.costUnknown))}
          </p>
          {run.usage.costUnknown ? (
            <p className="text-meta text-status-warning">
              {t('Actual cost unknown for {count} record{suffix}.', {
                count: run.usage.unknownCostRecordCount,
                suffix: run.usage.unknownCostRecordCount === 1 ? '' : 's',
              })}
            </p>
          ) : null}
        </SummaryTile>
      ...
      {run.failure !== null ? (
        <section
          aria-label={t('Failure detail')}
          className="rounded-lg border border-status-error bg-surface p-4"
        >
          <h2 className="text-sm font-semibold text-status-error">
            {t(PIPELINE_STAGE_LABEL[run.failure.stage])} {t('failed')} — {run.failure.category}
          </h2>
          <p className="mt-2 whitespace-pre-wrap font-mono text-meta text-text-primary">
            {run.failure.safeDetail}
          </p>
          <p className="mt-2 text-sm text-text-secondary">
            {t('Stages that completed are shown below; stages after the failure never started. Fix the cause, then retry — a retry always creates a new attempt and never overwrites this one.')}
          </p>
        </section>
      ) : null}

      <Section title={t('Review Summary')}>
        <p className="text-sm text-text-primary">{run.gate.summary}</p>
      </Section>

      <Section title={t('Required Dimensions')}>
        {run.dimensions.length === 0 ? (
          <p className="text-sm text-text-secondary">
            {t('No dimensions were scheduled by the Review Policy for this attempt.')}
          </p>
        ) : (
            ...
            {run.dimensions.map((dimension) => {
              ...
              return (
                ...
                >
                  ...
                  <span className={`text-meta ${status.className}`}>{t(status.label)}</span>
                  <span className="ml-auto flex items-center gap-3 text-meta text-text-secondary tabular-nums">
                    {dimension.elapsedMs !== null ? (
                      <span>{t(formatDurationMs(dimension.elapsedMs))}</span>
                    ) : null}
                    <span>
                      {dimension.findingCount} {t(dimension.findingCount === 1 ? 'finding' : 'findings')}
                      {dimension.blockingFindingCount > 0
                        ? ` · ${dimension.blockingFindingCount} ${t('blocking')}`
                        : ''}
                    </span>
                  </span>
                </li>
      ...
      <Section title={t('Review Pipeline')}>
        <PipelineStages stages={run.pipeline} />
      </Section>

      <Section title={t('Provider & Data Disclosure')}>
        <dl className="flex flex-col gap-2 text-sm">
          ...
          <div className="flex flex-wrap items-center gap-x-2">
            <dt className="text-text-secondary">{t('Provider health')}</dt>
            <dd className="text-text-primary">
              {health.status === 'healthy'
                ? t('Healthy at {timestamp}', { timestamp: formatTimestamp(health.observedAt) })
                : health.status === 'last_call_failed'
                  ? t('Last call failed at {timestamp}', { timestamp: formatTimestamp(health.observedAt) })
                  : t('Not tested')}
            </dd>
          </div>
        </dl>
