...
export function BypassRiskDialog({ open, run, onClose, onConfirm }: BypassRiskDialogProps) {
  ...
  return (
    <Dialog
      open={open}
      title="Accept the risk and bypass the gate?"
      description="Bypass means accepting the risk. It does not mean the finding was resolved."
      onClose={close}
    >
      <div className="flex flex-col gap-3 text-sm text-text-secondary">
        <div className="rounded-md border border-status-warning/40 bg-surface-subtle p-3">
          <p className="font-medium text-text-primary">
            ...
            {blockingFindings.length === 1 ? 'finding' : 'findings'} accepted as risk
          </p>
        ...
        <p>
          If confirmed, the gate becomes <strong>Passed with Bypass</strong> and this decision
          records your reason. The findings stay open and remain visible as accepted risk.
        </p>
